<div>
    @include('livewire.origin-create-form')
    {{-- @include('livewire.origin-edit-form' ) --}}
    @include('layouts.header')
    @include('livewire.origin-view-form')
    <section class="p-5">
        <div class="">
                ...
                <div class="overflow-x-auto relative">
                    ...
                    <nav class="flex px-5 py-3 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
                        ...
                        <li aria-current="page">
                            ...
                            <span class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400"> Origins</span>
                            </div>
                        </li>
                        </ol>
                    </nav>
                    ...
                    <table  class="w-full text-sm text-left text-gray-500 dark:text-gray-400 hadow-2xl sm:rounded-lg">
                            ...
                            @foreach($vendors as $vendor)
                                <tr class="bg-white dark:bg-gray-800">
                                    <td  class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                        @if($show==1 && $vendor['vendorId'] == $vendorCode)
                                            <button id="dropdownButton" wire:click.prevent="cancel" data-dropdown-toggle="dropdown" class="inline-block text-gray-500 dark:text-gray-400 rounded-full text-sm p-1.5 hover:bg-gray-200" type="button">
                                    ...
                                    <td class="py-4 px-6 font-small text-gray-900 whitespace-nowrap dark:text-white">
                                        @if($show==1 && $vendor['vendorId'] == $vendorCode)
                                            <div>
                                            @if($select==1)
                                    ...
                                    <td  class="py-4 px-6 font-small text-gray-900 whitespace-nowrap dark:text-white">
                                        ...
                                        @else
                                            {{$vendor->vendorLeadTime}}<span> Months</span>
                                        @endif
                                    </td>
                                    <td  class="py-4 px-6 font-small text-gray-900 whitespace-nowrap dark:text-white">
                                        @if($show==1 && $vendor['vendorId'] == $vendorCode)
                                        ...
                                        @else
                                            {{$vendor->vendorLotSize}}<span> Unit</span>
                                        @endif
                                    </td>
                                    <td  class="py-4 px-6 font-small text-gray-900 whitespace-nowrap dark:text-white">
                                        @if($show==1 && $vendor['vendorId'] == $vendorCode)
                                            <div>
                                                <input type="text" wire:model="mot" id="vendorname" class="inputBox text-center bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required >
                                                @error('mot')<span class="text-red-600">{{$message}}</span>@enderror
                                            </div>
                                        @else
                                            {{$vendor->MOT}}
                                        @endif
                                    ...
                                    <td class="py-4 px-6 font-small text-gray-900 whitespace-nowrap dark:text-white">
                                        {{
                                        \Carbon\Carbon::parse($vendor->vendorCreated)->format('F d Y')
                                        }}
                                    </td>
                                    <td class="py-4 px-6 font-small text-gray-900 whitespace-nowrap dark:text-white">
                                        {{\Carbon\Carbon::parse($vendor->vendorUpdate)->diffForHumans()}}
                                    </td>
                                    <td class="py-4 px-6 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        @if($show==1 && $vendor['vendorId'] == $vendorCode)
                                                ...
                                                <div class="flex items-center justify-center p-2">
                                                      ...
                                                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" wire:click.prevent="update()" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 text-green-700 transition ease-in-out delay-75 hover:-translate-y-1  hover:scale-110 cursor-pointer">
                                        ...
                                        @else
                                        <div>
                                            <button  class="inline-flex  text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 shadow-2xl" type="button"  wire:click.prevent="select({{$vendor->vendorId}})">   
                                                <svg class="w-6 h-6" fill="none" class="" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>                             
                                            <span class="ml-2 flex items-center">Edit</span>
                                            </button>
                                        </div>
                                        @endif
...
{{-- wire:click="edit({{$vendor['vendorId']}})" --}}