...
<!-- Main modal -->
<div wire:ignore.self id="authentication-modal" tabindex="-1" aria-hidden="true" class=" hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full justify-center items-center">
    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="py-6 px-6 lg:px-8">
                <h3 class="mb-4 text-xl font-medium text-gray-900 dark:text-white">Create a Production</h3>
                <form class="space-y-6">
                        ...
                        focus:text-gray-700 focus:bg-white border-blue-600 focus:outline-none"  aria-label="Default select example" name="type">
                            ...
                            @else
                                ...
                                @foreach($list1 as $sta)
                                    <option value="{{$sta->modelId}}">1{{$sta->modelName}}</option>
                                @endforeach
                            @endif
                        </select>
                    ...
                    <div>
                        <label for="memo" class="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">Production Quantity</label>
                        <div class="flex justify-between gap-x-4">
                            <button wire:click.prevent ="add" class="hover:border-black hover:bg-red-600 hover:text-white text-lg w-1/3">+</button>
                            <input type="number" wire:model="num" min="0" max="9999" value="{{ old('username') }}" name="username" class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg  focus:ring-blue-500 focus:border-blue-500 block w-1/3 p-2.5 dark:bg-gray-600 dark:border-gray-500 text-center text-lg dark:placeholder-gray-400 dark:text-white inputBox" placeholder="" >
                    ...
                    <button type="submit" wire:click.prevent="store()"  class="w-full text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Create</button>
                </form>
            </div>
        </div>
    </div>
